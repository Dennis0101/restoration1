...
verifyRouter.post('/verify', rateLimit, urlencoded({ extended: true }), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!cap?.success) {
      console.warn('hCaptcha failed', { ip, guildId, userId, errs: cap['error-codes'] });
    ...
    // 여기서 추가 필터(해외/VPN/부계정 등) 넣을 수 있음
    const passed = true;
    ...
    await prisma.verificationLog.create({
      data: {
        ...
        passed, reason: passed ? null : 'policy_blocked'
      }
    });

    // 역할 부여는 실패하더라도 전체 흐름은 계속
