...
export function renderCaptcha(
  ...
  guildId: string,
  userIdHint?: string,
  action: string = '/verify' // ← self-action 기본값
) {
  const safeGuild = String(guildId ?? '');
  const safeUser  = userIdHint ? String(userIdHint) : '';
...
<div class="card">
  ...
  <form method="POST" action="${action}">
    <input type="hidden" name="guildId" value="${safeGuild}">
    ${safeUser ? `<input type="hidden" name="userId" value="${safeUser}">` : ''}
